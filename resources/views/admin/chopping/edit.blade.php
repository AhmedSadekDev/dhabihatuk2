        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title m-0">{{ __('admin.editChopping') }}</h4>
                </div>
                <div class="card-body">
                    <form class="needs-validation" action="{{ route('Chopping.update') }}" method="POST"
                        enctype="multipart/form-data" id="product-form">
                        @csrf
                        @method('put')
                        @include('layouts.session')
                        <input type="hidden" name="chipping_id" value="{{ $chopping->id }}" />
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name_ar') is-invalid @enderror"
                                        ...
                                        value="{{ $chopping->name_ar }}" required>
                                    @error('name_ar')
                            ...
                            <div class="col-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name_en') is-invalid @enderror"
                                        ...
                                        value="{{ $chopping->name_en }}" required>
                                    @error('name_en')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                        ...
                        <button class="btn btn-primary" type="submit">{{ __('admin.editChopping') }}</button>
                    </form>
                </div>
            </div>
            <!-- end card -->
