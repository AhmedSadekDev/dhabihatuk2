                            ...
                            @foreach ($products as $count => $product)
                                <tr data-id="{{ $count + 1 }}">
                                    ...
                                    <td class="align-middle"><img
                                            src="{{ asset('Admin/images/products/' . $product->images[0]->image) }}"
                                            alt="{{ __('admin.image') }}" style="width: 100px;" /></td>
                                    <td class="align-middle">{{ $product->title() }}</td>
                                    <td class="align-middle">{!! mb_strimwidth($product->desc(), 0, 50, ',...') !!}</td>
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            @can('editProduct')
                                                <form class="d-inline ml-2" action="{{ route('products.verify') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        @if ($product->status == 1) title="{{ __('admin.hide') }}" @else title="{{ __('admin.showIcon') }}" @endif>
                                                        <i class="@if ($product->status == 1) fas fa-eye-slash @else fas fa-eye @endif"
                                                            style="color:white"></i>
                                                    </button>
                                                </form>
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content modal-content-demo">
                                                        ...
                                                        <div class="modal-footer">
                                                            <form class="d-inline" action="{{ route('products.delete') }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('Delete')
